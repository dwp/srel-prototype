...
module.exports = function (folderForViews, urlPrefix, router) {
  ...
  router.post('/live-version01/your-details/your-name-answer', function (req, res) {
    var hcpnameoption = req.session.data['hcpname']

    // Check whether the variable matches a condition
    if (hcpnameoption == "No") {
      // 
      res.redirect('/live-version01/your-details/your-name')
    } else {
      req.session.data['HCPfirstname'] = "Nelson"
      req.session.data['HCPlastname'] = "Noah"
      //
      res.redirect('/live-version01/your-details/role');
    }
  })

  //Delete your details/ 

  router.post('/live-version01/dashboard/your-account-delete', function (req, res) {
    ...
    if (deletedetailsoption == "Yes") {
      ...
      res.redirect('/live-version01/dashboard/your-account-delete-banner')
    } else {
      res.redirect('/live-version01/save-your-details/check-your-details');
    }
  })
  ...
  router.get('/live-version01/usertestingoption', function (req, res) {
    ...
    if (journeyoptions === "Journey 2" || journeyoptions === "Journey 3") {
      ...
      res.redirect('/live-version01/sr1-form-sections');
    }
  });

  router.get('/live-version01/index', function (req, res) {
    const data = req.session.data || {};
    const journeyoptions = data['journeyoptions'];

    // Intercept only if coming from confirmation page
    const referer = req.get('Referer') || '';
    const cameFromConfirmation = referer.includes('/live-version01/confirmation-page');

    if (
      cameFromConfirmation &&
      ["Journey 1", "Journey 2", "Journey 3"].includes(journeyoptions)
    ) {
      res.redirect('/live-version01/usertesting');
    } else {
  ...
  router.post('/confirm-your-details-options', function (req, res) {
    ...
    if (confirmyourdetails === "No") {
      res.redirect('live-version01/your-details/confirm-your-name');
    } else {
      res.redirect('live-version01/sr1-form-sections-testing');
    }
  });
  ...
  router.post('/confirm-organisation-address-options', function (req, res) {
    ...
    if (confirmyourdetails === "No") {
      res.redirect('live-version01/your-details//save-your-new-details');
    } else {
      res.redirect('live-version01/your-details/save-your-details');
    }
  });
  ...
  router.post('/live-version01/save-your-details/your-name-answer', function (req, res) {
    var hcpnameoption = req.session.data['hcpname']

    // Check whether the variable matches a condition
    if (hcpnameoption == "No") {
      // 
      res.redirect('/live-version01/save-your-details/your-name')
    } else {
      req.session.data['HCPfirstname'] = "Nelson"
      req.session.data['HCPlastname'] = "Noah"
      //
      res.redirect('/live-version01/save-your-details/role');
    }
  })

  router.post('/live-version01/diagnosis/have-other-diagnosis-answer', function (req, res) {
    var relevantdiagnosisoption = req.session.data['RelevantDiagnosis']

    // Check whether the variable matches a condition
    if (relevantdiagnosisoption == "Yes") {
      // 
      res.redirect('/live-version01/diagnosis/other-diagnosis')
    } else {
      res.redirect('/live-version01/diagnosis/aware-of-diagnosis');
    }
  })

  router.post('/live-version01/diagnosis/other-diagnosis-saved/answer', function (req, res) {
    var relevantdiagnosisoption = req.session.data['RelevantDiagnosis']

    // Check whether the variable matches a condition
    if (relevantdiagnosisoption == "Yes") {
      // 
      res.redirect('/live-version01/diagnosis/other-diagnosis-saved')
    } else {
      res.redirect('/live-version01/diagnosis/aware-of-diagnosis-saved');
    }
  })

  //Creating a list//

  router.post('/live-version01/patient-details/find-patient-address-answer', function (req, res) {
    ...
    res.redirect('/live-version01/patient-details/patient-address-results')
  }
  )
