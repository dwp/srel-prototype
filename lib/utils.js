...
function renderPathWithQuery (path, data, res, next) {
  console.log('renderPathWithQuery. data: ', data);
  // Try to render the path
  res.render(path, data, function (error, html) {
    if (!error) {
      // Success - send the response
      res.set({ 'Content-type': 'text/html; charset=utf-8' })
      res.end(html)
    ...
    if (!path.endsWith('/index')) {
      // Maybe it should be renderPathWithQuery
      renderPath(path + '/index', res, next)
      return
    }
    // We got template not found both times - call next to trigger the 404 page
...
exports.matchRoutes = function (req, res, next) {
  ...
  if (path.includes('task-list-sr1-form') || path.includes('edit-incomplete-form-saved')) {
    var data = req.query;
    renderPathWithQuery(path, data, res, next);
  } else {
